...
let productid = document.querySelector("#productid");
productid.addEventListener("focusout", () => {
	console.log(`Product Id: ${productid.value}`);
	let productprice = document.querySelector("#productprice");
	let productname = document.querySelector("#productname");

	fetch(`${window.origin}/api/newsell/${productid.value.trim()}/price/get/`, {
		method: "POST",
	}).then((response) => {
		...
		response.json().then((data) => {
			...
			localStorage.setItem("currentproduct", productname.value);
...
// WHEN SUBMIT THE PRODUCT NAME, QUANTITY AND PRICE
form.addEventListener("submit", (e) => {
	e.preventDefault();

	// RESET VALUE OF PRODUCT NAME AFTER SUBMIT
	...
	// FOCUS PRODUCT ID AFTER SUBMIT FORM
